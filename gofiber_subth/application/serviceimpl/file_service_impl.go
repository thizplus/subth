...
func NewFileService(fileRepo repositories.FileRepository, userRepo repositories.UserRepository, storage ports.Storage) services.FileService {
...
func (s *FileServiceImpl) UploadFile(ctx context.Context, userID uuid.UUID, fileHeader *multipart.FileHeader, options *dto.UploadFileRequest) (*models.File, error) {
	...
	if options != nil && options.CustomPath != "" {
		// Use custom path approach - ใช้ path ตรงๆ ไม่เพิ่ม UUID
		...
		// ถ้า custom_path มี extension (เช่น .jpg) ใช้เป็น path เต็ม
		// ถ้าไม่มี extension ให้เพิ่ม unique filename
		if filepath.Ext(validatedPath) != "" {
			cdnPath = validatedPath
		} else {
	...
	url, err := s.storage.Upload(ctx, cdnPath, file, mimeType)
	if err != nil {
...
func (s *FileServiceImpl) DeleteFile(ctx context.Context, fileID uuid.UUID) error {
	...
	err = s.storage.Delete(ctx, file.CDNPath)
	if err != nil {
