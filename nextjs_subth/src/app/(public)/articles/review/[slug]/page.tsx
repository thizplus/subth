...
// JS จะโหลดเมื่อ component เข้า viewport แต่ HTML ยังคง render จาก server
...
export default async function ReviewArticlePage({ params }: PageProps) {
  ...
  if (isV3Content(content)) {
    return <ArticlePageV3 article={article as any} locale="th" />;
  }
        ...
        <div id="cinematography" className="scroll-mt-20">
          ...
          {content.galleryImages && content.galleryImages.length > 4 && (
            <InlineGallery
              ...
              videoCode={article.videoCode}
              columns={2}
              caption="ฉากเด่นในเรื่อง"
            />
          )}
        </div>

        {/* Gallery - แสดงภาพที่เหลือ (ภาพ 7+) */}
