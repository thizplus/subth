...
const authors: Record<string, {
  ...
  "subth-editorial": {
    ...
    role: "Editorial Team",
    bio: "The SubTH Editorial Team consists of experienced writers and analysts with over 5 years of expertise in content review and analysis. Every article undergoes a rigorous fact-checking and quality assurance process before publication, ensuring readers receive accurate and comprehensive information.",
    expertise: ["Video Reviews", "Content Analysis", "Subtitles", "Cast Information"],
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!author) {
    return { title: "Author Not Found | SubTH" };
  }

  return {
    title: `${author.name} - Author | SubTH`,
    description: author.bio.slice(0, 160),
    alternates: {
      canonical: `https://subth.com/en/author/${slug}`,
      languages: {
...
export default async function AuthorPageEN({ params }: PageProps) {
  const { slug } = await params;
  const author = authors[slug];
  ...
  const authorSchema = {
    ...
    "@type": "Person",
    name: author.name,
    url: `https://subth.com/en/author/${author.slug}`,
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span>Joined {new Date(author.joinedDate).toLocaleDateString("en-US", { year: "numeric", month: "long" })}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <FileText className="h-4 w-4" />
                <span>{author.articleCount.toLocaleString()} articles</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-gradient-to-b from-muted/50 to-transparent p-6">
          ...
          <p className="text-muted-foreground mb-4">
            View all articles written by {author.name}
          </p>
          <Link
            href="/en/articles"
